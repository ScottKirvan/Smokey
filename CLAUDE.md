...
- **Traffic chart** — 14-day aggregate (days 1–14; day 0 is always empty because GitHub's traffic API is day-aggregated UTC with no real-time endpoint). X-axis label: "yest" for the most recent bucket. Note: GitHub's traffic data itself can lag by more than one day — "yesterday" showing 0 is not necessarily a bug, confirmed by cross-checking against `views.csv` on `traffic-log` (see below), which can also be missing the most recent day or two.
- **Traffic history (full range)** — collapsible section below the 14-day chart, expanded/fetched lazily on click. Reads `views.csv` from the `traffic-log` branch (`raw.githubusercontent.com/ScottKirvan/Smokey/traffic-log/views.csv`) — a daily snapshot accumulated by a separate weekly GitHub Actions workflow (`log-traffic.yml`, using the `TRAFFIC_PAT` repo secret) that isn't bounded by GitHub's 14-day live-API window. Filtered to `S.repos` (currently-monitored repos), aggregated by calendar date, rendered with a linear (not log-scaled) x-axis since the range keeps growing. No PAT involved — the CSV is a public file, fetched directly; doesn't touch the "PAT sent only to api.github.com" constraint. Cached in-memory after first load; invalidated when the monitored repo list changes in Settings.
